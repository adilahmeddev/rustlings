...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> {
        let mut err: Option<IntoColorError> = None;
        arr.iter().for_each(|a|{
            if a.clone()>255 || a.clone()<0{
                err = Some(IntoColorError::IntConversion);
            }
            
        });
        if err.is_some(){
            return Err(IntoColorError::from(err.unwrap()));
        }
